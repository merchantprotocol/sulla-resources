...
```
sulla <account_id>/gcal '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gcal"}'
```
...
### List resources
```bash
sulla <account_id>/gcal '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gcal '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
