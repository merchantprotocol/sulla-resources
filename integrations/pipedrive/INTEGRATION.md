...
```
sulla <account_id>/pipedrive '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"pipedrive"}'
```
...
### List resources
```bash
sulla <account_id>/pipedrive '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/pipedrive '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
