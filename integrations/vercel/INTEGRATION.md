...
```
sulla <account_id>/vercel '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"vercel"}'
```
...
### List resources
```bash
sulla <account_id>/vercel '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/vercel '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
