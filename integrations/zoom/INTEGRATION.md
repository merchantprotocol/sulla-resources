...
```
sulla <account_id>/zoom '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"zoom"}'
```
...
### List resources
```bash
sulla <account_id>/zoom '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/zoom '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
