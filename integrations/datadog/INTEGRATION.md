...
```
sulla <account_id>/datadog '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"datadog"}'
```
...
### List resources
```bash
sulla <account_id>/datadog '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/datadog '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
