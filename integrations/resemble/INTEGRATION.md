...
```
sulla <account_id>/resemble '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"resemble"}'
```
...
### List resources
```bash
sulla <account_id>/resemble '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/resemble '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
