...
```
sulla <account_id>/typeform '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"typeform"}'
```
...
### List resources
```bash
sulla <account_id>/typeform '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/typeform '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
