...
```
sulla <account_id>/gohighlevel '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gohighlevel"}'
```
...
### List resources
```bash
sulla <account_id>/gohighlevel '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gohighlevel '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
