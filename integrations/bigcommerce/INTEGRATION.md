...
```
sulla <account_id>/bigcommerce '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"bigcommerce"}'
```
...
### List resources
```bash
sulla <account_id>/bigcommerce '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/bigcommerce '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
