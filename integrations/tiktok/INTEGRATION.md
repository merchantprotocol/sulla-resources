...
```
sulla <account_id>/tiktok '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"tiktok"}'
```
...
### List resources
```bash
sulla <account_id>/tiktok '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/tiktok '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
