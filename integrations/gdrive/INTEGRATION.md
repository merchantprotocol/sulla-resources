...
```
sulla <account_id>/gdrive '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gdrive"}'
```
...
### List resources
```bash
sulla <account_id>/gdrive '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gdrive '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
