...
```
sulla <account_id>/btcpayserver '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"btcpayserver"}'
```
...
### List resources
```bash
sulla <account_id>/btcpayserver '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/btcpayserver '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
