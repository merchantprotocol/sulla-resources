...
```
sulla <account_id>/mailgun '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"mailgun"}'
```
...
### List resources
```bash
sulla <account_id>/mailgun '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/mailgun '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
