...
```
sulla <account_id>/freshdesk '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"freshdesk"}'
```
...
### List resources
```bash
sulla <account_id>/freshdesk '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/freshdesk '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
