...
```
sulla <account_id>/linkedin '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"linkedin"}'
```
...
### List resources
```bash
sulla <account_id>/linkedin '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/linkedin '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
