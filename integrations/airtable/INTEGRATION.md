...
```
sulla <account_id>/airtable '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"airtable"}'
```
...
### List resources
```bash
sulla <account_id>/airtable '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/airtable '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
