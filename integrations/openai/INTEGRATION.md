...
```
sulla <account_id>/openai '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"openai"}'
```
...
### List resources
```bash
sulla <account_id>/openai '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/openai '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
