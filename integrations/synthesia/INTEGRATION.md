...
```
sulla <account_id>/synthesia '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"synthesia"}'
```
...
### List resources
```bash
sulla <account_id>/synthesia '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/synthesia '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
