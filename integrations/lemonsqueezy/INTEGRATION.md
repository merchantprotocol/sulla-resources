...
```
sulla <account_id>/lemonsqueezy '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"lemonsqueezy"}'
```
...
### List resources
```bash
sulla <account_id>/lemonsqueezy '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/lemonsqueezy '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
