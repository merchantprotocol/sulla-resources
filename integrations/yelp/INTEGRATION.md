...
```
sulla <account_id>/yelp '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"yelp"}'
```
...
### List resources
```bash
sulla <account_id>/yelp '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/yelp '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
