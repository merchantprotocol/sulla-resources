---
slug: comfyui
name: comfyui-server-v1
type: proxy
auth_type: none
---

# comfyui-server-v1 Integration
...
```
sulla <account_id>/comfyui '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"comfyui"}'
```
...
### List resources
```bash
sulla <account_id>/comfyui '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/comfyui '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
