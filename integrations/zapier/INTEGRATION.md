...
```
sulla <account_id>/zapier '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"zapier"}'
```
...
### List resources
```bash
sulla <account_id>/zapier '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/zapier '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
