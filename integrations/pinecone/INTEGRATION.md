...
```
sulla <account_id>/pinecone '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"pinecone"}'
```
...
### List resources
```bash
sulla <account_id>/pinecone '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/pinecone '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
