...
```
sulla <account_id>/notion '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"notion"}'
```
...
### List resources
```bash
sulla <account_id>/notion '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/notion '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
