---
slug: cloudinary-video
name: cloudinary-video-v1
type: proxy
auth_type: basic
---

# cloudinary-video-v1 Integration
...
```
sulla <account_id>/cloudinary-video '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"cloudinary-video"}'
```
...
### List resources
```bash
sulla <account_id>/cloudinary-video '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/cloudinary-video '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
