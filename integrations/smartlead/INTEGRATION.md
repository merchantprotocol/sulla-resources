...
```
sulla <account_id>/smartlead '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"smartlead"}'
```
...
### List resources
```bash
sulla <account_id>/smartlead '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/smartlead '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
