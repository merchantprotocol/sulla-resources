...
```
sulla <account_id>/close '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"close"}'
```
...
### List resources
```bash
sulla <account_id>/close '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/close '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
