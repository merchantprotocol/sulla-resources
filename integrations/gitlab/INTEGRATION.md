...
```
sulla <account_id>/gitlab '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gitlab"}'
```
...
### List resources
```bash
sulla <account_id>/gitlab '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gitlab '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
