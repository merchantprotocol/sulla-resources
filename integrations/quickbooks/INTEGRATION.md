...
```
sulla <account_id>/quickbooks '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"quickbooks"}'
```
...
### List resources
```bash
sulla <account_id>/quickbooks '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/quickbooks '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
