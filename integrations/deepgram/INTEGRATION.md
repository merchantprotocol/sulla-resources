...
```
sulla <account_id>/deepgram '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"deepgram"}'
```
...
### List resources
```bash
sulla <account_id>/deepgram '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/deepgram '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
