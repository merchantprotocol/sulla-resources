...
```
sulla <account_id>/linear '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"linear"}'
```
...
### List resources
```bash
sulla <account_id>/linear '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/linear '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
