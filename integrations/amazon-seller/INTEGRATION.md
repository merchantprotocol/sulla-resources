...
```
sulla <account_id>/amazon-seller '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"amazon-seller"}'
```
...
### List resources
```bash
sulla <account_id>/amazon-seller '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/amazon-seller '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
