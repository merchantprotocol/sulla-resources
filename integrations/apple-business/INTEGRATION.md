---
slug: apple-business
name: apple-business-connect-v1
type: proxy
auth_type: bearer
---

# apple-business-connect-v1 Integration
...
```
sulla <account_id>/apple-business '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"apple-business"}'
```
...
### List resources
```bash
sulla <account_id>/apple-business '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/apple-business '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
