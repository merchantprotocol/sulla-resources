...
```
sulla <account_id>/whisper '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"whisper"}'
```
...
### List resources
```bash
sulla <account_id>/whisper '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/whisper '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
