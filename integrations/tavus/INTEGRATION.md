...
```
sulla <account_id>/tavus '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"tavus"}'
```
...
### List resources
```bash
sulla <account_id>/tavus '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/tavus '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
