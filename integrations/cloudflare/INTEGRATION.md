...
```
sulla <account_id>/cloudflare '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"cloudflare"}'
```
...
### List resources
```bash
sulla <account_id>/cloudflare '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/cloudflare '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
