...
```
sulla <account_id>/plaid '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"plaid"}'
```
...
### List resources
```bash
sulla <account_id>/plaid '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/plaid '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
