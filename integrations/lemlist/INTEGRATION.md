...
```
sulla <account_id>/lemlist '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"lemlist"}'
```
...
### List resources
```bash
sulla <account_id>/lemlist '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/lemlist '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
