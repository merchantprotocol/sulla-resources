...
```
sulla <account_id>/anthropic '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"anthropic"}'
```
...
### List resources
```bash
sulla <account_id>/anthropic '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/anthropic '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
