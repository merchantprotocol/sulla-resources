...
```
sulla <account_id>/minimax '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"minimax"}'
```
...
### List resources
```bash
sulla <account_id>/minimax '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/minimax '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
