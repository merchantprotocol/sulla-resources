...
```
sulla <account_id>/klaviyo '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"klaviyo"}'
```
...
### List resources
```bash
sulla <account_id>/klaviyo '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/klaviyo '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
