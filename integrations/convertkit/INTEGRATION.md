...
```
sulla <account_id>/convertkit '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"convertkit"}'
```
...
### List resources
```bash
sulla <account_id>/convertkit '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/convertkit '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
