...
```
sulla <account_id>/resend '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"resend"}'
```
...
### List resources
```bash
sulla <account_id>/resend '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/resend '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
