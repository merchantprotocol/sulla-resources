...
```
sulla <account_id>/play-ht '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"play-ht"}'
```
...
### List resources
```bash
sulla <account_id>/play-ht '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/play-ht '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
