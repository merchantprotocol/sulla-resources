...
```
sulla <account_id>/descript '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"descript"}'
```
...
### List resources
```bash
sulla <account_id>/descript '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/descript '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
