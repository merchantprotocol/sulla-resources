...
```
sulla <account_id>/apollo '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"apollo"}'
```
...
### List resources
```bash
sulla <account_id>/apollo '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/apollo '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
