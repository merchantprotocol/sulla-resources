...
```
sulla <account_id>/mcp-dataripple '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"mcp-dataripple"}'
```
...
### List resources
```bash
sulla <account_id>/mcp-dataripple '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/mcp-dataripple '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
