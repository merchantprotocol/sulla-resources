...
```
sulla <account_id>/assemblyai '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"assemblyai"}'
```
...
### List resources
```bash
sulla <account_id>/assemblyai '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/assemblyai '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
