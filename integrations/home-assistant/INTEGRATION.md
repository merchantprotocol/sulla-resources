...
```
sulla <account_id>/home-assistant '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"home-assistant"}'
```
...
### List resources
```bash
sulla <account_id>/home-assistant '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/home-assistant '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
