...
```
sulla <account_id>/freshsales '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"freshsales"}'
```
...
### List resources
```bash
sulla <account_id>/freshsales '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/freshsales '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
