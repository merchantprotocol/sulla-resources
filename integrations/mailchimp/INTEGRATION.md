...
```
sulla <account_id>/mailchimp '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"mailchimp"}'
```
...
### List resources
```bash
sulla <account_id>/mailchimp '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/mailchimp '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
