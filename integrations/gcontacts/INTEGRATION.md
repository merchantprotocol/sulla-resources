---
slug: gcontacts
name: google-people-v1
type: proxy
auth_type: oauth2
---

# google-people-v1 Integration
...
```
sulla <account_id>/gcontacts '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gcontacts"}'
```
...
### List resources
```bash
sulla <account_id>/gcontacts '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gcontacts '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
