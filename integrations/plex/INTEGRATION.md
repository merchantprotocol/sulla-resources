...
```
sulla <account_id>/plex '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"plex"}'
```
...
### List resources
```bash
sulla <account_id>/plex '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/plex '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
