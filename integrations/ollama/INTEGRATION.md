...
```
sulla <account_id>/ollama '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"ollama"}'
```
...
### List resources
```bash
sulla <account_id>/ollama '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/ollama '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
