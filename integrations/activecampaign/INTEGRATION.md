...
```
sulla <account_id>/activecampaign '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"activecampaign"}'
```
...
### List resources
```bash
sulla <account_id>/activecampaign '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/activecampaign '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
