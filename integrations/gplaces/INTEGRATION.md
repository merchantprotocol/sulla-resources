...
```
sulla <account_id>/gplaces '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gplaces"}'
```
...
### List resources
```bash
sulla <account_id>/gplaces '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gplaces '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
