...
```
sulla <account_id>/calendly '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"calendly"}'
```
...
### List resources
```bash
sulla <account_id>/calendly '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/calendly '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
