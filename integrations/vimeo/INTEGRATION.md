...
```
sulla <account_id>/vimeo '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"vimeo"}'
```
...
### List resources
```bash
sulla <account_id>/vimeo '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/vimeo '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
