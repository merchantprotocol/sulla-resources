...
```
sulla <account_id>/sentry '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"sentry"}'
```
...
### List resources
```bash
sulla <account_id>/sentry '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/sentry '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
