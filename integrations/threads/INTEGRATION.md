...
```
sulla <account_id>/threads '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"threads"}'
```
...
### List resources
```bash
sulla <account_id>/threads '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/threads '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
