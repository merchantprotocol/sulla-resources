...
```
sulla <account_id>/invideo '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"invideo"}'
```
...
### List resources
```bash
sulla <account_id>/invideo '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/invideo '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
