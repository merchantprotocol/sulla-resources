...
```
sulla <account_id>/runway '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"runway"}'
```
...
### List resources
```bash
sulla <account_id>/runway '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/runway '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
