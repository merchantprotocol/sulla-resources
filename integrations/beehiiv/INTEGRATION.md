...
```
sulla <account_id>/beehiiv '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"beehiiv"}'
```
...
### List resources
```bash
sulla <account_id>/beehiiv '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/beehiiv '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
