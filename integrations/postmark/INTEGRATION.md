...
```
sulla <account_id>/postmark '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"postmark"}'
```
...
### List resources
```bash
sulla <account_id>/postmark '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/postmark '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
