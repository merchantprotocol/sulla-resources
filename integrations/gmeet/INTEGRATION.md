...
```
sulla <account_id>/gmeet '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gmeet"}'
```
...
### List resources
```bash
sulla <account_id>/gmeet '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gmeet '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
