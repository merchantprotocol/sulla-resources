---
slug: gbusiness
name: google-business-profile-v1
type: proxy
auth_type: oauth2
---

# google-business-profile-v1 Integration
...
```
sulla <account_id>/gbusiness '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gbusiness"}'
```
...
### List resources
```bash
sulla <account_id>/gbusiness '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gbusiness '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
