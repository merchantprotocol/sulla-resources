...
```
sulla <account_id>/vaultwarden '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"vaultwarden"}'
```
...
### List resources
```bash
sulla <account_id>/vaultwarden '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/vaultwarden '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
