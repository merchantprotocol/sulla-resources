...
name: microsoft-graph-teams-v1
type: proxy
auth_type: oauth2
---

# microsoft-graph-teams-v1 Integration
...
```
sulla <account_id>/microsoft-teams '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"microsoft-teams"}'
```
...
### List resources
```bash
sulla <account_id>/microsoft-teams '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/microsoft-teams '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
