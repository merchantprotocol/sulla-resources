...
```
sulla <account_id>/pinterest '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"pinterest"}'
```
...
### List resources
```bash
sulla <account_id>/pinterest '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/pinterest '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
