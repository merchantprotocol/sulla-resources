...
```
sulla <account_id>/heygen '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"heygen"}'
```
...
### List resources
```bash
sulla <account_id>/heygen '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/heygen '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
