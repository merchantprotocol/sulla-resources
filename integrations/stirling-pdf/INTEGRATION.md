...
```
sulla <account_id>/stirling-pdf '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"stirling-pdf"}'
```
...
### List resources
```bash
sulla <account_id>/stirling-pdf '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/stirling-pdf '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
