...
name: stability-video-v2beta
type: proxy
auth_type: bearer
---

# stability-video-v2beta Integration
...
```
sulla <account_id>/stability-video '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"stability-video"}'
```
...
### List resources
```bash
sulla <account_id>/stability-video '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/stability-video '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
