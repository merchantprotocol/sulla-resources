...
```
sulla <account_id>/woocommerce '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"woocommerce"}'
```
...
### List resources
```bash
sulla <account_id>/woocommerce '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/woocommerce '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
