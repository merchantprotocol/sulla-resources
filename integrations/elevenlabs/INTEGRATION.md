...
```
sulla <account_id>/elevenlabs '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"elevenlabs"}'
```
...
### List resources
```bash
sulla <account_id>/elevenlabs '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/elevenlabs '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
