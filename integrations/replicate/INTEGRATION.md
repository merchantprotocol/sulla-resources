...
```
sulla <account_id>/replicate '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"replicate"}'
```
...
### List resources
```bash
sulla <account_id>/replicate '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/replicate '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
