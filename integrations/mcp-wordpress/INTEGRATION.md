...
```
sulla <account_id>/mcp-wordpress '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"mcp-wordpress"}'
```
...
### List resources
```bash
sulla <account_id>/mcp-wordpress '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/mcp-wordpress '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
