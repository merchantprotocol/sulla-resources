...
```
sulla <account_id>/plane '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"plane"}'
```
...
### List resources
```bash
sulla <account_id>/plane '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/plane '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
