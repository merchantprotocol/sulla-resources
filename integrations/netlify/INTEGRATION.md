...
```
sulla <account_id>/netlify '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"netlify"}'
```
...
### List resources
```bash
sulla <account_id>/netlify '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/netlify '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
