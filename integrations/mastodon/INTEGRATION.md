...
```
sulla <account_id>/mastodon '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"mastodon"}'
```
...
### List resources
```bash
sulla <account_id>/mastodon '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/mastodon '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
