...
```
sulla <account_id>/todoist '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"todoist"}'
```
...
### List resources
```bash
sulla <account_id>/todoist '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/todoist '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
