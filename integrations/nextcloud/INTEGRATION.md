...
```
sulla <account_id>/nextcloud '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"nextcloud"}'
```
...
### List resources
```bash
sulla <account_id>/nextcloud '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/nextcloud '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
