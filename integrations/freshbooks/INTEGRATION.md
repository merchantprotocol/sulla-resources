...
```
sulla <account_id>/freshbooks '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"freshbooks"}'
```
...
### List resources
```bash
sulla <account_id>/freshbooks '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/freshbooks '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
