...
```
sulla <account_id>/kling '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"kling"}'
```
...
### List resources
```bash
sulla <account_id>/kling '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/kling '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
