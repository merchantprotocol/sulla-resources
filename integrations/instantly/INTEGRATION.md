...
```
sulla <account_id>/instantly '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"instantly"}'
```
...
### List resources
```bash
sulla <account_id>/instantly '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/instantly '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
