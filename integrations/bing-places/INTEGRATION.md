...
```
sulla <account_id>/bing-places '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"bing-places"}'
```
...
### List resources
```bash
sulla <account_id>/bing-places '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/bing-places '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
