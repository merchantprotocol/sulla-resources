...
```
sulla <account_id>/gdocs '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"gdocs"}'
```
...
### List resources
```bash
sulla <account_id>/gdocs '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/gdocs '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
