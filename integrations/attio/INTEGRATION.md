...
```
sulla <account_id>/attio '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"attio"}'
```
...
### List resources
```bash
sulla <account_id>/attio '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/attio '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
