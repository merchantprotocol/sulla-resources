...
```
sulla <account_id>/customer-io '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"customer-io"}'
```
...
### List resources
```bash
sulla <account_id>/customer-io '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/customer-io '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
