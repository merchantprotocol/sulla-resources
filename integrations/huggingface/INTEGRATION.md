...
```
sulla <account_id>/huggingface '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"huggingface"}'
```
...
### List resources
```bash
sulla <account_id>/huggingface '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/huggingface '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
