...
```
sulla <account_id>/brevo '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"brevo"}'
```
...
### List resources
```bash
sulla <account_id>/brevo '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/brevo '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
