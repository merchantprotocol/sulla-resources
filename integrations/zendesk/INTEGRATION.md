...
```
sulla <account_id>/zendesk '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"zendesk"}'
```
...
### List resources
```bash
sulla <account_id>/zendesk '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/zendesk '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
