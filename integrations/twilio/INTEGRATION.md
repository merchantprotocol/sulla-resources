...
```
sulla <account_id>/twilio '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"twilio"}'
```
...
### List resources
```bash
sulla <account_id>/twilio '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/twilio '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
