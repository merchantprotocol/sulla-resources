...
```
sulla <account_id>/intercom '<json_body>'
```
...
```bash
sulla vault/list_accounts '{"account_type":"intercom"}'
```
...
### List resources
```bash
sulla <account_id>/intercom '{"method":"GET","path":"/"}'
```

### Create a resource
```bash
sulla <account_id>/intercom '{
  "method": "POST",
  "path": "/resource",
  "body": {"key": "value"}
}'
